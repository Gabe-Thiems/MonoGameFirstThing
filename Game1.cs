...
public class Game1 : Game
{
    ...
    public const int gravityStrength = 1;
    public const int gravityForce = 9;
    ...
    public class PhysicsGameObject
    {
        ...
        public PhysicsGameObject(Texture2D texture, Vector2 position, int weight, bool isForceAffected, Vector2 force, Vector2 velocity, float collider_width, float collider_height, bool collider_isSolid, string tag, bool isCollided )
        {
    ...
    public void Physics(List<PhysicsGameObject> objects)
    {
        ...
        {
            gameObject.force.Y += gravityStrength;
        }   
        ...
        foreach (PhysicsGameObject gameObject in objects) 
        {
            ...
            if (collider != null && collider.Item1.collider_isSolid == true)
            {
                collisions.Add(collider);
            }

        }
        //If you are colliding with something, you can assume an equal and opposite force is pushing back (not quite sound but works for now)
        foreach (Tuple<PhysicsGameObject, Vector2> collider in collisions)
        {
            if (collider.Item2 == new Vector2(-1, 0))
            {
                collider.Item1.force.X += -collider.Item1.force.X;
            }
            ...
            else if (collider.Item2 == new Vector2(0, 1))
            {
                ...
                collider.Item1.force.Y += -collider.Item1.force.Y;
            }
            else if (collider.Item2 == new Vector2(0, -1))
            {
                collider.Item1.force.Y += -collider.Item1.force.Y;
        ...
        foreach (PhysicsGameObject gameObject in objects)
        {
            gameObject.velocity += gameObject.force;

            //limits
            
            if (gameObject.velocity.Y > gravityForce) {gameObject.velocity.Y = gravityForce;}
        }
    ...
    protected override void LoadContent()
    {
        ...
        rectangleObject = new PhysicsGameObject(
            ...
            Vector2.Zero,
            Vector2.Zero,
            (pixelScale * 2f), //20F is for debug, 1.0F is closer to accurate
            (pixelScale * 1f),
            true,
            "rectangle",
            false
    ...
    protected override void Draw(GameTime gameTime)
    {
        ...
        _spriteBatch.Draw(recTexture, rectangleObject.position, null, Color.White, 0f, Vector2.Zero, 2f, SpriteEffects.None, 0f);
        _spriteBatch.End();
